...
object TimeUsage extends TimeUsageInterface {
  ...
  def timeUsageSummary(
    ...
    val ageProjection: Column = expr("if (teage >= 15 and teage <= 22, 'young', if (23 <= teage and teage <= 55, 'active', 'elder'))").as("age")
  ...
  def timeUsageGroupedSqlQuery(viewName: String): String =
    "select working, sex, age, round(avg(primaryNeeds),1) as primaryNeeds, round(avg(work),1) as work, round(avg(other),1) as other from " +
    viewName + " group by working, sex, age order by working, sex, age"
