...
//    if (b.legalNeighbors.isEmpty) LazyList.empty
//    else {
//      val more = for {
//        neighbor <- b.legalNeighbors
//        neighborWithHistory = (neighbor._1, neighbor._2 :: history)
//      } yield neighborWithHistory
//      (b, history) #:: neighborsWithHistory(neighborWithHistory._1, neighborWithHistory._2)
//    }
    for {
      neighbor <- b.legalNeighbors.to(LazyList)
    } yield (neighbor._1, neighbor._2 :: history)

  def neighborsWithHistoryAlt(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    (for {
  ...
  def newNeighborsOnlyAlt(neighbors: LazyList[(Block, List[Move])],
                       explored: Set[Block]): LazyList[(Block, List[Move])] =
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if (initial.isEmpty) LazyList.empty
      ...
      val more = for {
        blockWithMove <- initial
        block = blockWithMove._1
        moves = blockWithMove._2
  ...
  def fromAlt(initial: LazyList[(Block, List[Move])],
           explored: Set[Block]): LazyList[(Block, List[Move])] = {
    if (initial.isEmpty) LazyList.empty
    else {
      val newPaths = for {
  ...
  lazy val pathsToGoalAlt: LazyList[(Block, List[Move])] = pathsFromStart.filter(x => done(x._1))
