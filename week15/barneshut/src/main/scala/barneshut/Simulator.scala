...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = {
    ...
    bound.minY = Math.min(boundaries.minY, body.y)
    bound.minX = Math.min(boundaries.minX, body.x)
    bound.maxY = Math.max(boundaries.maxY, body.y)
    bound.maxX = Math.max(boundaries.maxX, body.x)
    bound
  }
