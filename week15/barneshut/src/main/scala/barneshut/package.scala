...
package object barneshut {
  ...
  extends Quad {
    ...
    def insert(b: Body): Quad = {
      // if size is less or equal to minimumSize,
      // Creates another Leaf with all the existing bodies and the new one.
      if (size <= minimumSize) Leaf(centerX, centerY, size, bodies :+ b)
      else {
        // If size of a Leaf is greater than a predefined constant minimumSize,
        // inserting an additional body into that Leaf quadtree creates a Fork
        // quadtree with empty children.
        // And ADDs all the bodies(foldLeft) into that Fork, including the new body,
    ...
    def updated(quad: Quad): Body = {
      ...
      def addForce(thatMass: Float, thatMassX: Float, thatMassY: Float): Unit = {
        ...
        if (dist > 1f) {
          ...
          val dforcex = dforce * xn // force on the x direction
          val dforcey = dforce * yn // force on the y direction
          netforcex += dforcex
          netforcey += dforcey
        }
      }
      ...
      def traverse(quad: Quad): Unit = (quad: Quad) match {
        ...
        case Fork(nw, ne, sw, se) =>
          ...
          if (quad.size / distance(quad.centerX, quad.centerY, x, y) < theta)
            ...
            //  if (x > quad.centerX)
            //  if (y > quad.centerY) traverse(se) else traverse(ne)
            //    else
            //  if (y > quad.centerY) traverse(sw) else traverse(nw)
            traverse(nw)
            traverse(ne)
            traverse(sw)
            traverse(se)
