...
class ReductionsSuite {
  ...
  @Test def `parBalance should work for string of length 2 and threshold 1`: Unit = {
    def check(input: String, threshold: Int, expected: Boolean): Unit =
      assert(parBalance(input.toArray,1) == expected ,s"parBalance should work for string of length ${input.length}")

    check(")(", 1, false)
  }

  @Test def `check downsweepSequential function`: Unit = {
    val input1: Array[Float] = Array(1.2f, 1.2f, 5.8f, 3.0f, 15.0f, 30.0f, 120.0f, 7.0f)
    val input2: Array[Float] = Array(1.2f, 1.2f, 5.8f, 3.0f, 15.0f, 30.0f, 120.0f, 7.0f)
    val output1: Array[Float] = new Array(8)
    val output2: Array[Float] = new Array(8)
    downsweepSequential(input1, output1, 0, 0, 8);
    downsweepSequentialAlt(input2, output2, 0, 0, 8);
    assert(output1.sameElements(output2), "downsweepSequential not implemented correctly")
  }
