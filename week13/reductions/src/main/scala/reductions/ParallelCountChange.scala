...
object ParallelCountChange extends ParallelCountChangeInterface {
  ...
  def parCountChange(money: Int, coins: List[Int], threshold: Threshold): Int = {
    // threshold does not take care when money <= 0 or coins.isEmpty, it may lead to recursive call.
  ...
  def moneyThreshold(startingMoney: Int): Threshold = {
    def threshold(currentMoney: Int, currentCoins: List[Int]): Boolean =
      if (currentMoney <= startingMoney * 2 / 3) true
      else false
    threshold
  }
  ...
  def totalCoinsThreshold(totalCoins: Int): Threshold = {
    def threshold(currentMoney: Int, currentCoins: List[Int]): Boolean =
      if (currentCoins.length <= totalCoins * 2 / 3) true
      else false
    threshold
  }
  ...
  def combinedThreshold(startingMoney: Int, allCoins: List[Int]): Threshold = {
    def threshold(currentMoney: Int, currentCoins: List[Int]): Boolean =
      if (currentMoney * currentCoins.length <= startingMoney * allCoins.length / 2) true
      else false
    threshold
  }
}
