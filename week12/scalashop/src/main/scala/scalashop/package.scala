...
//    val maxOfI = clamp(x + radius, 0, src.width - 1)
//    var j = clamp(y - radius, 0, src.height - 1)
//    val maxOfJ = clamp(y - radius, 0, src.height - 1)
//
//    while (i <= maxOfI) {
//      while (j <= maxOfJ) {
//        yield src(i, j)
...
//  }
  ...
  def boxBlurKernel(src: Img, x: Int, y: Int, radius: Int): RGBA = {
    val raw = for {
      ...
      point = src(i, j) // apply function triggered
    } yield point

    val avg = raw.foldLeft[List[Int]](List(0, 0, 0, 0))(
      (t,c) =>  List(t(0) + red(c), t(1) + green(c), t(2) + blue(c), t(3) + alpha(c))
    ).map(_ / raw.size)
